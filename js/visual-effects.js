...
;(() => {
    class ParticleTrail {
      ...
      findInteractiveElements() {
        ...
        this.interactiveElements = [
          ...document.querySelectorAll('a, button, .card, .project, [data-interactive]')
        ]
      }
      ...
      createParticle() {
        ...
        if (this.config.useRainbowColors) {
          ...
          const colorScheme = this.isDarkMode 
            ? this.config.colorSchemes.dark 
            : this.config.colorSchemes.light
        ...
        const size = Math.random() * 
          (this.config.particleSizeRange[1] - this.config.particleSizeRange[0]) + 
      ...
      updateParticles(deltaTime) {
        ...
        for (let i = this.particles.length - 1; i >= 0; i--) {
          ...
          if (this.config.useRainbowColors && p.color.colorSpeed) {
            ...
            const baseIndex = Math.floor(p.color.colorPosition / (2 * Math.PI) * this.config.colorSchemes.rainbow.length) % this.config.colorSchemes.rainbow.length;
      ...
      drawParticles() {
        ...
        for (const p of this.particles) {
          ...
          ctx.shadowColor = this.isDarkMode 
            ? this.config.colorSchemes.dark.shadow 
            : this.config.colorSchemes.light.shadow
