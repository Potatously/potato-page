...
;(() => {
  ...
  function shouldShowTerminal() {
    ...
    // Verificar si hay una preferencia guardada para no mostrar la terminal
    const skipTerminal = sessionStorage.getItem("skipTerminal") === "true";
    ...
    // 3. No hay preferencia guardada para omitir la terminal
    const shouldShow = (isReload || isDirectEntry) && !skipTerminal;

    // Si vamos a mostrar la terminal, actualizar el timestamp
    if (shouldShow) {
      sessionStorage.setItem("lastTerminalTimestamp", Date.now().toString());
    } else {
      // Si no mostramos la terminal, guardar preferencia para esta sesión
      sessionStorage.setItem("skipTerminal", "true");
    }
